...
import style from "./Columnists.module.css"
                        ...
                        <a className={style.slide} href={slide.link} key={index} draggable={false} onMouseDown={startTouch} onTouchStart={startTouch} onClick={click} >
                            <div className={style.slideContainer}>
                                <div className={style.slideImgContainer}>
                                    <img draggable={false} className={style.slideImg} src={slide.img} alt="" />
                                </div>
                                <h1 className={style.slideName}>{slide.name}</h1>
                                <p className={style.slideJobTitle}>{slide.jobTitle}</p>
                            </div>
                        </a>
                    )}
                </div>
...
export default function Columnists(props) {
    return (
        <div className={style.infoSlider}>
            <h1 className={style.headTitle}>Наши колумнисты</h1>
